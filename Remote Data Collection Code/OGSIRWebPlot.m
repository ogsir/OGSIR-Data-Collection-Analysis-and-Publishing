...
cd('C:/Users/Yowza/Google Drive/OGSIR/Data');

diary('Log.txt')                                 % open Log and append command window outputs

DataTable = readtable('DataTable.csv');          % read csv into table of variables
...
WebData = DataTable(OneWeekAgo:Newest,:);                   % Save current web data to variable

%% ---- Trim egregiously bad data ---- %%
% commented out until I test to make sure it behaves as expected

% 
% if WebData.AirTmp(:)<(-15) || WebData.AirTmp(:)>45        % trim spurious temps
%     WebData.AirTmp(:) = 'NAN'
% end
% 
% if WebData.BarPress(:)<970 || DataTable.BarPress(:)>1040  % trim spurious pressures
%     WebData.BarPress(:) = 'NAN'
% end

%% Save data for web plots

writetable(WebData,'WebData.csv');            % write clean csv

%% Send text messages to notify of important events
% commented out until I can test to make sure it behaves as expected
%
% if (all(DataTable.Rain(TwelveHoursAgo:Previous)<1) && ( DataTable.Rain(Newest) > 1)    % If it's just starting to rain, send a text
...
% This code was modified from an autogenerated script copied from
%   http://plot.ly//~OGSIR.OSU/13
% That code was generated through the site's interactive plot GUI, by:
%   -uploading a sample data set to plot.ly 
%   -http://help.plot.ly/make-a-line-graph-or-scatter-plot/
% Plotly-generated comments:
...
% Defines data streams and how they are displayed
% Contents:
%
% 1. DATA SERIES structure variables, each listed separately
%    These each include:
%       data variable in vector form
%       display formatting information: marker, color, which axes to display on, etc.
%
% 2. Data variable call, listing each of the preceding structures
%
% 3. DISPLAY FORMATTING structure variable, including:
%    General formatting
%    x axis formatting: position, size, color, etc.
%    y axis formatting: position, size, color, etc.
%    legend formatting
%    formatting of any additional x or y axes
% 
% 4. Plotly function call, referencing data and display formatting from above
%    This function sends the above structural information to the website,
%    where it is encoded for display using the open-source D3 javascript library.

WeatherPlot.AirTemperature = struct(...
...
WeatherPlot.SolarRadiation = struct(...
  'x', {  DataTable.Time(OneWeekAgo:Newest) }, ...
...
WeatherPlot.BarometricPressure = struct(...
  'x', {  DataTable.Time(OneWeekAgo:Newest) }, ...
...
WeatherPlot.RelativeHumidity = struct(...
  'x', {  DataTable.Time(OneWeekAgo:Newest) }, ...
...
WeatherPlot.WindDirection = struct(...
  'x', {  DataTable.Time(OneWeekAgo:Newest) }, ...
...
WeatherData = {WeatherPlot.AirTemperature, ...
    WeatherPlot.SolarRadiation, ...
    WeatherPlot.Rain, ...
    WeatherPlot.BarometricPressure, ...
    WeatherPlot.RelativeHumidity, ...
    WeatherPlot.WindDirection, ...
    WeatherPlot.WindSpeed};
...
layout = struct(...
    ...
    'yaxis7', struct(...                                       % y axis 7 %
      ...
      'tickfont', struct('color', 'rgb(159, 197, 232)'), ...
